...
lp.checkLVal = function (expr, binding) {
  ...
  switch (expr.type) {
    ...
    default:
      return this.dummyIdent("Incorrect expression type");
  }
};
...
lp.parseSubscripts = function (base, start, noCalls, startIndent, line) {
  for (;;) {
    ...
    if (this.eat(_.tokTypes.dot)) {
      var node = this.startNodeAt(start);
      node.object = base;
      if (this.curLineStart != line && this.curIndent <= startIndent && this.tokenStartsLine()) node.property = this.dummyIdent("Missing member identifier");else node.property = this.parsePropertyAccessor() || this.dummyIdent("Missing member identifier");
      node.computed = false;
...
lp.parseVar = function (noIn) {
  ...
  if (!node.declarations.length) {
    var decl = this.startNode();
    decl.id = this.dummyIdent("Missing variable identifier");
    node.declarations.push(this.finishNode(decl, "VariableDeclarator"));
  }
