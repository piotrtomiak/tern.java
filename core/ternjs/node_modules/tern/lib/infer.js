...
(function(root, mod) {
  ...
  function similarAVal(a, b) {
    var typeA = a.getType(false), typeB = b.getType(false);
    if (!typeA || !typeB) return true;
    return similarType(typeA, typeB);
  }

  function similarType(a, b) {
    if (!a) return b;
    if (!a || a == b) return a;
    if (!b) return a;
    ...
    if (a.constructor == Arr) {
      ...
      if (!innerB || similarType(innerA, innerB)) return b;
    } else if (a.constructor == Obj) {
      var propsA = 0, propsB = 0, same = 0;
      for (var prop in a.props) {
        propsA++;
        if (prop in b.props && similarAVal(a.props[prop], b.props[prop]))
          same++;
      }
      ...
      if (a.args.length != b.args.length ||
          !a.args.every(function(tp, i) { return similarAVal(tp, b.args[i]); }) ||
          !similarAVal(a.retval, b.retval) || !similarAVal(a.self, b.self))
        return false;
      return a;
    } else {
      return false;
  ...
  var simplifyTypes = exports.simplifyTypes = function(types) {
    ...
    outer: for (var i = 0; i < types.length; ++i) {
      ...
      for (var j = 0; j < found.length; j++) {
        var similar = similarType(tp, found[j]);
        if (similar) {
          found[j] = similar;
          continue outer;
        }
  ...
  var qualityFactors = [0.005, 0.002, 0.001, 0.0005, 0.0002, 0.0001 ];
  
  exports.setQualityLevel = function(level) {
	  if (level < 0) level = 0;
	  if (level >= qualityFactors.length) level = qualityFactors.length - 1;
	  reduceMaxWorkDepth = qualityFactors[level];
  }
  
  var baseMaxWorkDepth = 30, reduceMaxWorkDepth = 0.0002;
