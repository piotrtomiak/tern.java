...
(function(root, mod) {
  ...
  function removeShebang(text) {
    if (text.charAt(0) == "#" && text.charAt(1) == "!") { // starts with shebang
      //fill the first line with empty chars
      var newLine = text.indexOf('\n');
      if (newLine > 0) {
        if (text.charAt(newLine - 1) == '\r') {
          newLine--;
        }
        return new Array(newLine + 1).join(' ') + text.slice(newLine);
      } else {
  ...
  var Server = exports.Server = function(options) {
    ...
    infer.setQualityLevel(options.quality);
  ...
  function doRequest(srv, doc, c) {
    ...
    if (queryType.noInfer) {
      noInferQuery(srv, timeBudget, function(err) {
        if (err) return c(err);
    ...
    analyzeAll(srv, timeBudget, function(err) {
      ...
      function run() {
        ...
        try {
          ...
          if (e.name != "TernError") console.error(e.stack);
          return c(e);
        }
        c(null, result);
      }
  ...
  function noInferQuery(srv, timeBudget, c) {
    if (srv.pending) return waitOnFetchNoInfer(srv, timeBudget, c);
    var e = srv.fetchError;
    if (e) { srv.fetchError = null; return c(e); }
    c();
  }
  
  function waitOnFetchNoInfer(srv, timeBudget, c) {
    var done = function() {
      srv.off("everythingFetched", done);
      clearTimeout(timeout);
      noInferQuery(srv, timeBudget, c);
    };
  ...
  function analyzeFile(srv, file) {
    infer.withContext(srv.cx, function() {
      ...
      try {
        ...
        console.error("Error when analysing file " + file.name);
        throw e;
      }
