(function(f){if(typeof exports==="object"&&typeof module!=="undefined"){module.exports=f()}else if(typeof define==="function"&&define.amd){define([],f)}else{var g;if(typeof window!=="undefined"){g=window}else if(typeof global!=="undefined"){g=global}else if(typeof self!=="undefined"){g=self}else{g=this}(g.acorn || (g.acorn = {})).sval = f()}})(function(){var define,module,exports;return (function e(t,n,r){function s(o,u){if(!n[o]){if(!t[o]){var a=typeof require=="function"&&require;if(!u&&a)return a(o,!0);if(i)return i(o,!0);var f=new Error("Cannot find module '"+o+"'");throw f.code="MODULE_NOT_FOUND",f}var l=n[o]={exports:{}};t[o][0].call(l.exports,function(e){var n=t[o][1][e];return s(n?n:e)},l,l.exports,e,t,n,r)}return n[o].exports}var i=typeof require=="function"&&require;for(var o=0;o<r.length;o++)s(r[o]);return s})({1:[function(_dereq_,module,exports){
"use strict";
...
lp.parseParenExpression = function () {
  ...
  if (this.expect(_.tokTypes.parenL)) {
	  var val = this.parseExpression();
	  if (!this.canEat(_.tokTypes.parenR)) {
...
lp.parseMaybeConditional = function (noIn) {
  ...
  if (this.eat(_.tokTypes.question)) {
    ...
    node.alternate = this.expect(_.tokTypes.colon) ? this.parseMaybeAssign(noIn) : this.dummyIdent("Incomplete conditional expression. Missing ':'.");
...
//        node.right = this.dummyIdent("Empty expression");
//      } else {
...
//    }
    ...
    if (this.eat(_.tokTypes.dot)) {
      var node = this.startNodeAt(start);
      node.object = base;
//      if (this.curLineStart != line && this.curIndent <= startIndent && this.tokenStartsLine()) node.property = this.dummyIdent("Missing member identifier");else 
...
lp.parseExprAtom = function () {
  ...
  switch (this.tok.type) {
    ...
    case _.tokTypes._yield:
      node = this.startNode();
      this.next();
      if (this.type == _tokentype.types.semi || this.canInsertSemicolon() || this.tok.type != _.tokTypes.star && !this.tok.type.startsExpr) {
        node.delegate = false;
        node.argument = null;
      } else {
...
//      line = this.curLineStart,
  ...
  while (!this.closes(close/*, indent + 1, line*/)) {
	if (this.canEat(_.tokTypes.comma)) {
    ...
    endsWithComma = this.eat(_.tokTypes.comma);
  }
  if (endsWithComma) {
      elts.push(allowEmpty ? null : this.dummyIdent("Missing expression"));
  }
...
},{"..":1,"./expression":2,"./state":5,"./statement":6,"./tokenize":7,"./visitErrorNodes":8}],4:[function(_dereq_,module,exports){
"use strict";
...
//    this.nextLineStart = this.lineEnd(this.curLineStart) + 1;
  ...
  SyntaxValidator.prototype.isUseStrict = function (stmt) {
  ...
  SyntaxValidator.prototype.expectContextual = function (name) {
    if (this.eatContextual(name)) return true;
    this.reportError("Expected name '" + name + "'");
    for (var i = 1; i <= 2; i++) {
      var ahead = this.lookAhead(i);
      if (ahead.type === _.tokTypes.name && ahead.value === name) {
        for (var j = 0; j < i; j++) {
          this.next();
        }return true;
      }
  ...
  SyntaxValidator.prototype.insertSemicolon = function () {
  ...
  SyntaxValidator.prototype.semicolon = function semicolon() {
    var isOk = this.eat(_.tokTypes.semi) || this.insertSemicolon();
    if (!isOk) {
...
//  SyntaxValidator.prototype.indentationAfter = function indentationAfter(pos) {
//    for (var count = 0;; ++pos) {
...
//  };
  ...
  SyntaxValidator.prototype.closes = function closes(closeTok/*, indent, line, blockHeuristic*/) {
    if (this.tok.type === closeTok || this.tok.type === _.tokTypes.eof) return true;
    return false;
    //return line != this.curLineStart && this.curIndent < indent && this.tokenStartsLine() && (!blockHeuristic || this.nextLineStart >= this.input.length || this.indentationAfter(this.nextLineStart) < indent);
  };

//  SyntaxValidator.prototype.tokenStartsLine = function tokenStartsLine() {
...
lp.parseStatement = function (declaration, topLevel) {
  ...
  switch (starttype) {
    ...
    case _.tokTypes._do:
      ...
      node.test = this.eat(_.tokTypes._while) ? this.parseParenExpression() : this.dummyIdent("missing while statement");
      this.semicolon();
...
lp.parseClass = function (isStatement) {
  var node = this.startNode();
  this.next();
  if (this.tok.type === _.tokTypes.name) node.id = this.parseIdent();else if (isStatement) node.id = this.dummyIdent("Missing class name");else node.id = null;
  node.superClass = this.eat(_.tokTypes._extends) ? this.parseExprSubscripts() : null;
...
//  }
  while (!this.closes(_.tokTypes.braceR/*, indent, line*/)) {
    if (this.eat(_.tokTypes.semi)) continue;
    var method = this.startNode();
    var isGenerator = this.eat(_.tokTypes.star);
    var isMaybeStatic = this.tok.type === _.tokTypes.name && this.tok.value === "static";
    ...
    method["static"] = isMaybeStatic && this.tok.type !== _.tokTypes.parenL && this.tok.type != _.tokTypes.braceL;
    if (method["static"]) {
        if (isGenerator) {
    ...
    if (!method.computed) {
        ...
        if (!isGenerator && key.type === "Identifier" && this.tok.type !== _.tokTypes.parenL && (key.name === "get" || key.name === "set")) {
            isGetSet = true;
    ...
    if (isGetSet && !_parseutil.isDummy(method.value.params)) {
        ...
        if (method.value.params.length !== paramCount) {
          ...
          var end = method.value.body ? method.value.body.start : method.value.end;
          if (method.kind === "get") 
...
lp.parseFunction = function (node, isStatement) {
  ...
  if (this.tok.type === _.tokTypes.name) node.id = this.parseIdent();else if (isStatement) node.id = this.dummyIdent("Missing function name");
  node.params = this.parseFunctionParams();
...
lp.lookAhead = function (n) {
  ...
  return this.ahead[n - 1];
...
},{"..":1,"./state":5}], 8:[function(_dereq_,module,exports){
	...
	lp.visitErrorNodes = function(program) {
		var parser = this;
...
},{"walk":9,"./state":5}], 9:[function(_dereq_,module,exports) {

	"use strict";

	module.exports = typeof walk != 'undefined' ? walk : require("acorn/dist/walk");
}, {}]},{},[3])(3)
});