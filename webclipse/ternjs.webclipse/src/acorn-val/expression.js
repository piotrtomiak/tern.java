...
import {tokTypes as tt} from "acorn/dist/acorn"
...
lp.parseParenExpression = function() {
  ...
  if (this.expect(tt.parenL)) {
    var val = this.parseExpression()
    if (!this.canEat(tt.parenR)) {
...
lp.parseMaybeConditional = function(noIn) {
  ...
  if (this.eat(tt.question)) {
    ...
    node.alternate = this.expect(tt.colon) ? this.parseMaybeAssign(noIn) : this.dummyIdent("Incomplete conditional expression. Missing ':'.")
...
lp.parseExprAtom = function() {
  ...
  case tt._yield:
    node = this.startNode()
    this.next()
    if (this.type == this.tok.type.semi || this.canInsertSemicolon() || this.tok.type != tt.star && !this.tok.type.startsExpr) {
      node.delegate = false
      node.argument = null
    } else {
...
lp.parseExprList = function(close, allowEmpty) {
  ...
  while (!this.closes(close)) {
    ...
    endsWithComma = this.eat(tt.comma)
  }
  if (endsWithComma) {
      elts.push(allowEmpty ? null : this.dummyIdent("Missing expression"))
  }
